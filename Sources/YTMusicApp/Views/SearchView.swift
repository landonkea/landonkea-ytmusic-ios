...
struct SearchView: View {
    ...
    /// Injected via @EnvironmentObject so child views can also access it.
    ...
    /// The current text in the search field. @State means this view "owns" it —
    /// when it changes, the view re-renders automatically.
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                ...
                // When SearchBar changes searchText, SearchView sees the update.
                SearchBar(
                    text: $searchText,
                ...
                // ── SEARCH FILTERS ─────────────────────────────────
                // Show filter buttons when a search has been performed.
                // Horizontal scroll of pill-shaped buttons: All, Songs, Videos, Albums.
                if isSearching {
                        ...
                        HStack(spacing: 10) {
                            ForEach(filters, id: \.name) { filter in
                                Button(action: {
                                    ...
                                    Text(filter.name)
                                        .font(.subheadline)
                                        .fontWeight(selectedFilter == filter.key ? .bold : .regular)
                ...
                if isSearching {
                    if apiClient.isLoading {
                        ...
                        // These look like the actual results but shimmer while loading
                        VStack(spacing: 0) {
                    ...
                    // State 4: Idle — show recent searches if available
    ...
    var body: some View {
        HStack {
            ...
            HStack {
                ...
                // Only shows if speech recognition is available on this device
            ...
            LazyVStack(spacing: 0) {
                ForEach(results) { result in
                    SearchResultRow(result: result)
                        ...
                        // Shows 4 options: Play, Play Next, Add to Queue, Download.
                        .contextMenu {
                            Button(action: {
                                playResult(result)
                            }) {
                ...
                ForEach(searches, id: \.self) { query in
                    Button(action: {
                        ...
                        HStack(spacing: 12) {
                            ...
                            .buttonStyle(.plain) // Prevents the tap from propagating
                        }
                        .padding(.horizontal)
                        .padding(.vertical, 10)
                    }
